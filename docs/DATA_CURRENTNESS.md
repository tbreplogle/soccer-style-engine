...
Statuses now apply league by league:

- `current`: raw and processed data look usable.
- `probably_current`: usable with caveats, often historical or offseason context.
- `season_completed`: the league appears complete and should not be called stale.
- `offseason`: the run date is after the season range.
...
Processed freshness compares only raw files relevant to the run. If processed data is older than those files, rerun normalization or let the daily pipeline normalize.
