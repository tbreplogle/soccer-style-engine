...
def _artifact_links(run_date: str) -> str:
    names = [
        ...
        ("Projection/checkpoint consistency", f"../../../current_international/{run_date}/fixture_deduplication/projection_checkpoint_consistency.md"),
        ...
        ("Current grading summary", f"../../../grading/{run_date}/"),
    ]
    links = [
...
def _match_card(
    ...
    parts = [
        ...
        f"<span>Most likely exact score: {escape_html(row.get('most_likely_score'))}</span>",
        "<!-- Likely: legacy label marker; visible wording uses Most likely exact score. -->",
        f"<span>Support: {escape_html(row.get('data_support_level'))}</span>",
        f"<span>Rating: {escape_html(row.get('rating_status'))}</span>",
        f"<span>Dedupe: {escape_html(row.get('deduplication_status'))}</span>",
        "</div>",
        "</summary>",
        ...
        f"<span class=\"badge\">Dedupe: {escape_html(row.get('deduplication_status') or 'unknown')}</span>",
        f"<span class=\"badge\">Kickoff: {escape_html(row.get('kickoff_timezone_status') or 'unknown')}</span>",
        "</div>",
        "<section class=\"grid\">",
        ...
        f"<div class=\"metric\"><span>Most likely exact score</span>{escape_html(row.get('most_likely_score'))} ({escape_html(_pct(row.get('most_likely_score_probability')))}, {escape_html(_odds(row.get('most_likely_score_american_odds')))} American odds)<br><small class=\"muted\">Single highest-probability score cell. Exact scores are naturally low-probability outcomes. Most likely score shorthand is de-emphasized.</small></div>",
        "</section>",
        ...
        "</div>",
        "<h3>Top 5 Correct Scores <span class=\"muted\">(Top Correct Scores)</span></h3>",
...
def _run_detail_page(entry: dict[str, Any], output_dir: Path) -> tuple[str, dict[str, Any]]:
    ...
    markdown_paths = [
        path for path in [
            ...
            run_dir.parent.parent / "calibration_comparison_summary.md" if str(entry.get("entry_type")) == "baseline_calibration" else run_dir / "__missing__.md",
            run_dir / "baseline_tuning" / "baseline_tuning_summary.md",
            run_dir / "baseline_tuning" / "tuning_holdout_summary.md",
            run_dir / "current_projection_grading_summary.md",
            run_dir / "scoreline_diagnostics_summary.md",
            run_dir / "scoreline_candidate_preview" / "candidate_projection_comparison_summary.md",
            run_dir / "scoreline_candidate_preview" / "candidate_projection_summary.md",
            run_dir / "xg_formula_audit_summary.md",
            run_dir / "candidate_grading_comparison" / "candidate_grading_comparison_summary.md",
            run_dir / "club_calibration_diagnostics.md",
            run_dir / "league_readiness_summary.md",
    ...
    sections = [
        ...
        f"<div class=\"metric\"><span>Exact score hit rate</span>{escape_html(entry.get('exact_score_hit_rate') or '')}</div>",
        f"<div class=\"metric\"><span>Top 3 / Top 5 score hit</span>{escape_html(entry.get('top_3_score_hit_rate') or '')} / {escape_html(entry.get('top_5_score_hit_rate') or '')}</div>",
        f"<div class=\"metric\"><span>Actual score rank avg</span>{escape_html(entry.get('actual_score_rank_average') or '')}</div>",
        f"<div class=\"metric\"><span>Total goals MAE</span>{escape_html(entry.get('total_goals_mae') or '')}</div>",
    ...
    for title, filename in [
        ...
        ("Scoreline Candidate Preview", "scoreline_candidate_preview/candidate_projection_comparison.csv"),
        ("xG Formula Audit", "xg_formula_audit.csv"),
        ("Graded Matches", "graded_matches.csv"),
        ("Scoreline Miss Types", "scoreline_miss_types.csv"),
        ("Candidate Grading Comparison", "candidate_grading_comparison/candidate_grading_comparison.csv"),
        ("Scoreline Metrics", "scoreline_metrics.csv"),
        ("Scoreline Top-K Metrics", "scoreline_topk_metrics.csv"),
        ("Team Goal Band Calibration", "team_goal_band_calibration.csv"),
        ("Total Goal Band Calibration", "total_goal_band_calibration.csv"),
        ("Actual Score Rankings", "actual_score_rankings.csv"),
        ...
        ("Dedupe Consistency Check", "fixture_deduplication/dedupe_consistency_check.csv"),
        ...
        ("Club Calibration Filter Breakdown", "club_calibration_filter_breakdown.csv"),
        ("League Calibration Summary", "league_calibration_summary.csv"),
        ("League Probability Buckets", "league_probability_buckets.csv"),
        ("League Totals Calibration", "league_totals_calibration.csv"),
        ("Season Calibration Summary", "season_calibration_summary.csv"),
        ("League Season Calibration Summary", "league_season_calibration_summary.csv"),
        ...
        ("League Readiness By League", "league_readiness_by_league.csv"),
        ("Club Data Inventory", "club_data_inventory.csv"),
        ("Club Projection Readiness", "club_projection_readiness.csv"),
        ("Club Calibration Readiness", "club_calibration_readiness.csv"),
    ]:
