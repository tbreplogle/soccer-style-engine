...
def _visible_generated_outputs() -> list[str]:
    try:
        ...
        return []
    bad_prefixes = ("data/raw/", "data/processed/", "outputs/reports/", "outputs/projections/", "outputs/runs/")
...
def run_operational_health_check() -> dict[str, Any]:
    ...
    for folder in ["data/raw/football-data", "data/processed", "outputs", "outputs/runs"]:
    ...
    commands = ["run-daily-pipeline", "explain-operational-defaults", "operational-health-check"]
